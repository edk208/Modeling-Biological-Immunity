...
local numEpochs           = 2;     --Number of times to run through dataset
...
local pvParameters = {
   ...
   column = {
      ...
      printParamsFilename                 = "MNistOne.params";
...
local file = io.open("./MNistOne.params", "w");
io.output(file);
pv.printConsole(pvParameters)
io.close(file);
-- The & makes it run without blocking execution
--os.execute("../../../python/draw -p -a " .. "./MNistOne.params &");
os.execute("mpirun -np 1 ../../../build/tests/BasicSystemTest/Release/BasicSystemTest -p MNistOne.params -t 4");
